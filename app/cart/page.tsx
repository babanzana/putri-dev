...
export default function CartPage() {
  ...
  const { items, updateQty, toggleSelect, selectAll, removeItem, selectedItems, totalPrice } =
    useCart();
      ...
      <div className="space-y-4">
        ...
        {items.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white/70 px-4 py-8 text-center text-sm text-slate-600">
            Keranjang kosong. <Link href="/catalog" className="text-amber-700 font-semibold">Cari produk</Link>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-3 sm:flex-col sm:items-end">
                  <div className="flex items-center rounded-xl border border-slate-200">
                    ...
                    <div className="px-4 py-2 text-sm font-semibold text-slate-900">{item.qty}</div>
                    <button
                  ...
                  <p className="text-[11px] text-slate-500">Stok max: {item.stock}</p>
                  <button

...
export default function CatalogDetailPage() {
  ...
  const goNext = () => setActiveIndex((prev) => (prev + 1) % product.images.length);
  const goPrev = () =>
              ...
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`relative h-16 w-full overflow-hidden rounded-lg border ${
                    activeIndex === idx ? "border-amber-300" : "border-slate-200"
                  }`}
                  type="button"
                >
          ...
          <div className="space-y-4 rounded-2xl bg-white/90 p-5 shadow-sm ring-1 ring-black/5 backdrop-blur">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center rounded-xl border border-slate-200">
                ...
                <div className="px-4 py-2 text-sm font-semibold text-slate-900">{qty}</div>
                <button
                  type="button"
                  className="px-3 py-2 text-sm font-semibold text-slate-700"
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-3 text-xs text-slate-600">
              <p className="font-semibold text-slate-800">Info pengiriman</p>
              <p>Estimasi tiba 1-3 hari untuk Jabodetabek, ongkir menyesuaikan kurir.</p>
              <p className="mt-2 font-semibold text-slate-800">Garansi</p>
              <p>Garansi toko 7 hari, retur jika barang cacat.</p>
            </div>
          </div>
        </div>

        {products.filter((p) => p.slug !== product.slug && p.category === product.category).length >
          0 && (
          <div className="space-y-2">
            <p className="text-sm font-semibold text-slate-900">Rekomendasi lain</p>
